...
local function createModel(opt)
   ...
   local function residual(nInputPlane, nOutputPlane, stride,type)
      ...
      -- if type ~= 'no_preact' and type ~= 'both_preact' then
      ...
      if type == 'last' then
         s:add(ConvolutionQ(nOutputPlane*2,512,3,3,1,1,1,1,opt.Wbits,opt.clipW))
         s:add(SBatchNorm(512))
      else
         s:add(ConvolutionQ(nOutputPlane*2,nOutputPlane,3,3,1,1,1,1,opt.Wbits,opt.clipW))
         s:add(nn.Replicate(2,1,3))                  
         if nOutputPlane == 64 then
            s:add(nn.Reshape(nOutputPlane*2,56,56,true)) 
         elseif nOutputPlane == 90 then
