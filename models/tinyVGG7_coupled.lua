...
local function createModel(opt)
   ...
   model:add(SBatchNorm(45))
   add_activation(model,opt.activation,opt.Abits)
   
   model:add(Convolution(45,45,3,3,1,1,1,1))
   model:add(Max(2,2,2,2))
   model:add(SBatchNorm(45))
   add_activation(model,opt.activation,opt.Abits)
   
   model:add(Convolution(45,90,3,3,1,1,1,1))
   model:add(Max(2,2,2,2))
   ...
   model:add(Convolution(90,336,4,4,4,4,0,0))
   model:add(SBatchNorm(336))
   add_activation(model,opt.activation,opt.Abits)
   
   model:add(Convolution(336,512,1,1,1,1,0,0))
