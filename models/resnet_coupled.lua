...
local function createModel(opt)
   ...
   local function shortcut(nInputPlane, nOutputPlane, stride, type)
      ...
      if useConv then
         ...
         return nn.Sequential()
            ...
            :add(Convolution(nInputPlane, nOutputPlane, 1, 1, 1, 1))
   ...
   local function residual(nInputPlane, nOutputPlane, stride,type)
      ...
      s:add(ConvolutionQ(nInputPlane,nOutputPlane,3,3,stride,stride,1,1,opt.Wbits,opt.clipW))
      ...
      if type == 'last' then
         s:add(ConvolutionQ(nOutputPlane,512,3,3,1,1,1,1,opt.Wbits,opt.clipW))
         s:add(SBatchNorm(512))
      else
         s:add(ConvolutionQ(nOutputPlane,nOutputPlane,3,3,1,1,1,1,opt.Wbits,opt.clipW))
         s:add(SBatchNorm(nOutputPlane))
      end
      return s
   end
   ...
   local function basicblock(n, stride, type)
      ...
      if type == '512' then
         return block
         :add(nn.ConcatTable()
            :add(residual(nInputPlane,n,stride,'last'))
            :add(shortcut(nInputPlane,n,stride,'last')))
